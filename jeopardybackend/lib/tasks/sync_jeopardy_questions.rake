namespace :jeopardy do
  ...
  task :sync_questions => :environment do
    ...
    JeopardyGame.jarchive_ids_to_sync.each do |target_game_id|

      puts "SYNCING GAME ID: #{target_game_id}"
      jeopardy_game = JeopardyGame.find_or_create_by(jarchive_id: target_game_id)
      ...
      if game_header.present?
        ...
        if single_jeopardy.present? && double_jeopardy.present?
          jeopardy_game.update_attributes(gamedate: game_date, game_exists: true)
          ...
          puts "SYNCED Game Data for ID: #{target_game_id}"
          puts "========================================"

        else

          jeopardy_game.update_attributes(game_exists: false)
